...
function validateRecorrenciaInput({ tipo, valor, categoria, diaDoMes, dataInicio, dataFim }) {
  const valorAusente = valor === undefined || valor === null || valor === '';
  const diaAusente = diaDoMes === undefined || diaDoMes === null || diaDoMes === '';
  if (!tipo || valorAusente || !categoria || diaAusente || !dataInicio) {
    return 'Campos obrigatórios: tipo, valor, categoria, dia do mês, data de início';
  }
